...
class TestUtils:
    ...
    def test_is_int_with_string(self):
        assert is_int('1') is True
        assert is_int('-1') is True
        assert is_int('a') is False
        assert is_int('') is False
        assert is_int('abcdefghij') is False
    ...
    def test_seconds_human_friendly(self):
        ...
        ten_hours_twentyfive_minutes_fiftynine_seconds = (
            one_hour * 10 + round(ten_minutes * 2.5) + 59
        )
        assert (
            seconds_human_friendly(
                ten_hours_twentyfive_minutes_fiftynine_seconds
            )
            == '10:25:59'
        )

        twentythree_hours_fiftynine_minutes_fiftynine_seconds = (
            one_hour * 23 + round(ten_minutes * 5.9) + 59
        )
        assert (
            seconds_human_friendly(
                twentythree_hours_fiftynine_minutes_fiftynine_seconds
